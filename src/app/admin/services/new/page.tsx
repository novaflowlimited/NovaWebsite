...
import type { PlanInterval, PricingType, ServiceCategory } from "@/types";
...
export default function AdminServiceNewPage() {
  ...
  >([{ name: "Starter", priceKes: 0, interval: "MONTHLY", features: "", isPopular: false }]);
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <Card>
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-navy">Slug override</label>
            <input
              className="mt-1 w-full rounded-lg border border-navy/15 px-3 py-2 text-sm"
              value={slugOverride}
              onChange={(e) => setSlugOverride(e.target.value)}
              placeholder={slugifyTitle(name)}
            />
          </div>
          <div>
            ...
            onClick={async () => {
              ...
              try {
                await apiFetch("/api/services", {
                  ...
                  body: JSON.stringify({
                    name,
                    slug: slugOverride.trim() || slugifyTitle(name),
                    tagline,
                    description,
                    icon,
                    category,
