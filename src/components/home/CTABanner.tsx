...
export function CTABanner({ settings }: { settings: SiteSettingsPayload }) {
  const { cta } = settings;
  return (
    <section className="bg-white py-12 md:py-16">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-orange via-orange to-orange-soft px-8 py-12 text-white shadow-xl shadow-orange/20 md:px-12 md:py-16">
          <div
            aria-hidden
          ...
          <div className="relative flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/80">{cta.eyebrow}</p>
              <h2 className="mt-2 text-3xl font-extrabold leading-tight tracking-tight md:text-4xl lg:text-5xl">
                {cta.title}
              </h2>
              <p className="mt-3 max-w-xl text-sm text-white/85 md:text-base">{cta.body}</p>
            </div>
            <Link
              href={cta.href}
            ...
            >
              {cta.button}
              <IconArrowRight className="h-4 w-4 shrink-0 text-navy transition-colors group-hover:text-white" aria-hidden />
            </Link>
          </div>
        </div>
      </div>
